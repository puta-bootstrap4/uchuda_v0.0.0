...
const VRInputComponent: React.FC = () => {
  ...
  useEffect(() => {
    if (isRunning) {
      console.log("Timer started"); // デバッグ用
      ...
      const pageTransitionTimer = setTimeout(() => {
        console.log("Transitioning to finish page"); // デバッグ用
        console.log("missCount:", missCountRef.current); // デバッグ用
        console.log("count:", countRef.current); // デバッグ用
        const params = new URLSearchParams();
        params.append("key1", `${missCountRef.current}`);
        params.append("key2", `${countRef.current}`);
        const href = `/finish/?${params}`;
        console.log("Navigating to:", href); // デバッグ用
        router.push(href);
      }, 41000); // 41秒後に実行
